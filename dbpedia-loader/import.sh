...
do
        resp2=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3  PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> INSERT {?s rdf:type ex:wiki_dbfr_equiv } WHERE {SELECT ?s ?y  FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE {?s ?p  ?y . FILTER NOT EXISTS { ?s rdf:type ex:wiki_dbfr_equiv }. FILTER(STRSTARTS(STR(?y), 'http://fr.dbpedia.org/') ) } LIMIT $limit};");
        echo "resp2 : $resp2";
        for graph in ${graph_list[@]}; do
            resp3=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3  PREFIX ex: <http://example.org/> WITH <$graph> DELETE { ?s ?p ?o } INSERT { ?y ?p ?o } WHERE {{SELECT ?s ?y FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE {?s owl:sameAs ?y. FILTER EXISTS { ?s rdf:type ex:wiki_dbfr_equiv }. FILTER(STRSTARTS(STR(?y), 'http://fr.dbpedia.org/') ) } LIMIT $limit }. {SELECT ?s ?p ?o FROM <$graph> WHERE {?s ?p ?o } } };");
            echo "$graph resp3: $resp3"
        done
        #resp4=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3 PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> INSERT { ?o_fr  owl:sameAs  ?s. ?o_fr dbo:wikiPageInterLanguageLink ?o_other. } WHERE {SELECT  ?s ?o_fr ?o_other FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis>  WHERE { ?s owl:sameAs ?o_fr. ?s owl:sameAs ?o_other. FILTER EXISTS { ?s rdf:type ex:wiki_dbfr_equiv }. FILTER ( ?o_other != ?o_fr). FILTER(STRSTARTS(STR(?o_fr), 'http://fr.dbpedia.org/'))} LIMIT $limit };")   
        #echo "resp4: $resp4";
        #resp5=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3 PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> DELETE {?s owl:sameAs ?o_fr. ?s owl:sameAs ?o_other} WHERE {SELECT  ?s ?o_fr ?o_other FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis>  WHERE { ?s owl:sameAs ?o_fr. ?s owl:sameAs ?o_other. FILTER EXISTS { ?s rdf:type ex:wiki_dbfr_equiv }. FILTER ( ?o_other != ?o_fr). FILTER(STRSTARTS(STR(?o_fr), 'http://fr.dbpedia.org/'))} . FILTER(STRSTARTS(STR(?o_fr), 'http://fr.dbpedia.org/') ) } LIMIT $limit };")   
        #echo "resp5: $resp5";
        resp45=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3 PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> INSERT { ?o_fr  owl:sameAs  ?s. ?o_fr dbo:wikiPageInterLanguageLink ?o_other. } DELETE {?s owl:sameAs ?o_fr. ?s owl:sameAs ?o_other} WHERE {SELECT  ?s ?o_fr ?o_other FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis>  WHERE { ?s owl:sameAs ?o_fr. ?s owl:sameAs ?o_other. FILTER EXISTS { ?s rdf:type ex:wiki_dbfr_equiv }. FILTER ( ?o_other != ?o_fr). FILTER(STRSTARTS(STR(?o_fr), 'http://fr.dbpedia.org/'))} LIMIT $limit };")   
        echo "resp45: $resp45";
done
echo '[STEP 2] CLEAN EACH NAMED GRAPH'
for graph in ${graph_list[@]}; do
 resp6=$( run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3 PREFIX ex: <http://example.org/> WITH <$graph> DELETE {?s ?p ?o}{{SELECT ?s ?p ?o FROM <$graph> WHERE { ?s ?p ?o} }. {SELECT ?s  FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { ?s owl:sameAs ?o_sameAs FILTER NOT EXISTS { ?s rdf:type ex:wiki_dbfr_equiv } } } };")
 echo "$graph resp6: $resp6";
done

echo '[STEP 3] CLEAN SAMEAS NAMED GRAPH'
resp7=$( run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3  PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> DELETE {?s ?p ?o}{{SELECT ?s ?p ?o FROM <$graph> WHERE { ?s ?p ?o} }. {SELECT ?s  FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { ?s owl:sameAs ?o_sameAs FILTER NOT EXISTS { ?s rdf:type ex:wiki_dbfr_equiv } } } };")
echo "resp7 : $resp7";
echo '[STEP 4] DELETE FLAG'
resp8=$( run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 3  PREFIX ex: <http://example.org/> WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> DELETE {?s rdf:type ex:wiki_dbfr_equiv}{{SELECT ?s  FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { ?s rdf:type ex:wiki_dbfr_equiv  } } };")
echo "resp8 : $resp8";
#### TO DO :
# Delete flag ?
# interwiki links ?
