...
do
    ...
    INSERT { tag-fr:WdtFrResource oa:hasTarget ?y. } \
    WHERE { SELECT ?y FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> \
    WHERE { ?s owl:sameAs ?y. FILTER NOT EXISTS { tag-fr:WdtFrResource oa:hasTarget ?y }. \
    ...
    for graph in ${graph_list[@]}; do
        ...
        do
            ...
            PREFIX oa: <http://www.w3.org/ns/oa#> \
               ...
               WHERE { ?y owl:sameAs ?s. FILTER EXISTS { tag-fr:WdtFrResource oa:hasTarget ?s }} \
               } . { \
    ...
    resp3=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 2 \
              PREFIX tag-fr: <http://fr.dbpedia.org/tag/> \
              PREFIX oa: <http://www.w3.org/ns/oa#> \
              WITH <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> \
              ...
              WHERE { ?s owl:sameAs ?y. FILTER EXISTS {  tag-fr:WdtFrResource oa:hasTarget ?y } \
              } LIMIT $limit};");
        ...
        resp4=$(run_virtuoso_cmd "SPARQL DEFINE sql:log-enable 2 \
              ...
              PREFIX oa: <http://www.w3.org/ns/oa#> \
               ...
               WHERE { SELECT ?s ?y ?p FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { \
                tag-fr:WdtFrResource oa:hasTarget ?y. \
               ?s owl:sameAs ?y. \
               ?s owl:sameAs ?p. \
               FILTER (?y != ?p ) \
               } LIMIT $limit };");
        resp_todo=$(run_virtuoso_cmd "SPARQL 
              PREFIX tag-fr: <http://fr.dbpedia.org/tag/> \
              PREFIX oa: <http://www.w3.org/ns/oa#> \
              SELECT COUNT(*) FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { tag-fr:WdtFrResource oa:hasTarget ?y. ?s owl:sameAs ?y. ?s owl:sameAs ?p. FILTER (?y != ?p ) };");
    ...
    do
      ...
      ################### SPARQL - INVERSE SAMEAS
        ...
        resp_todo2=$(run_virtuoso_cmd "SPARQL SELECT COUNT(?s) FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE { tag-fr:WdtFrResource oa:hasTarget ?y. ?s owl:sameAs ?y. };");
