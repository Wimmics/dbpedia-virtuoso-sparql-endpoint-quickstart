...
for graph in ${graph_list_dbpedia[@]}; do
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> INSERT INTO <${DOMAIN}/graph/metadata> { <$graph> void:classPartition [void:class ?class ; void:entities ?count ] . } WHERE { \        
        SELECT ?class (count(?instance) AS ?count) WHERE { \
        	SELECT DISTINCT ?class ?instance FROM <$graph> WHERE { \
	        ?instance ?p ?o. { \ 
	        select ?instance ?c FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> WHERE { ?instance a ?class } } \
	        } } GROUP BY ?class \
        };"
