...
for graph in ${graph_list_dbpedia[@]}; do
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph> void:classPartition [ void:class ?class ; void:entities ?count ] . } \
         WHERE { SELECT ?class (count(?instance) AS ?count) WHERE { \
            SELECT DISTINCT ?class ?instance FROM <$graph> WHERE { ?instance ?p ?o. \
                { select ?instance ?class FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
                    WHERE { ?instance a ?class } } } } GROUP BY ?class };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         WITH <${DOMAIN}/graph/statistics> \
         DELETE { <$graph> void:classPartition ?bn. \
          ?bn void:class ?c. ?bn void:properties ?x. } \
         WHERE { <$graph> void:classPartition ?bn. \
          ?bn void:class ?c. ?bn void:properties ?x. FILTER (isBlank(?bn))};"
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph> void:classPartition [ void:class ?c ; void:properties ?x ] . } \
         WHERE { SELECT (COUNT(DISTINCT ?p) AS ?x) ?c FROM <$graph> WHERE { ?s ?p ?o. \
          { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
            WHERE { ?s a ?c } } } GROUP BY ?c  };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         WITH <${DOMAIN}/graph/statistics> \
         DELETE { <$graph> void:classPartition ?bn. \
          ?bn void:class ?c. ?bn void:classes ?x } \
         WHERE { <$graph> void:classPartition ?bn. \
         ...
         INSERT INTO <${DOMAIN}/graph/statistics> { \
                ...
                WHERE { SELECT (COUNT(DISTINCT ?d) AS ?x) ?c  FROM <$graph> WHERE { ?s ?p ?o. \
                 { select ?s ?c ?d FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
                    WHERE {  ?s a ?c , ?d } } } GROUP BY ?c };"
        echo "- distinct subject per classes";

        ################### SPARQL - CLASS PARTITION - Nb of triples distincts subjects associated to a class
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         WITH <${DOMAIN}/graph/statistics> \
         DELETE { <$graph> void:classPartition  ?bn. ?bn void:class ?c. ?bn void:distinctSubjects ?x  } \
         WHERE { <$graph> void:classPartition  ?bn. ?bn void:class ?c. ?bn void:distinctSubjects ?x. FILTER (isBlank(?bn)) };"
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph> void:classPartition [ void:class ?c ; void:distinctSubjects ?x ] . } \
         WHERE { SELECT (COUNT(DISTINCT ?s) AS ?x) ?c FROM <$graph> WHERE { ?s ?p ?o. \
          { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
            WHERE { ?s a ?c } } } GROUP BY ?c };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         WITH <${DOMAIN}/graph/statistics> \
         DELETE { <$graph>  void:classPartition  ?bn. ?bn void:class ?c. ?bn void:distinctObjects ?x  } \
         WHERE { <$graph>  void:classPartition  ?bn. ?bn void:class ?c. ?bn void:distinctObjects ?x. FILTER (isBlank(?bn)) . };"
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph>  void:classPartition [ void:class ?c ; void:distinctObjects ?x ] . } \
         WHERE { SELECT (COUNT(DISTINCT ?o) AS ?x) ?c FROM <$graph> WHERE { ?s ?p ?o. \
          { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
            WHERE { ?s a ?c } }} GROUP BY ?c };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph> void:classPartition [ void:class ?c ; void:propertyPartition [void:property ?p ; void:triples ?x ] ]. } \
         WHERE { SELECT ?c (COUNT(?o) AS ?x) ?p FROM <$graph> WHERE { ?s ?p ?o. \
          { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_mappings_instance-types> \
            WHERE { ?s a ?c } } } GROUP BY ?c ?p };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         ...
         WHERE {  <$graph> void:classPartition ?bn1. \
          ?bn1 void:class ?c. \
            ?bn1 void:propertyPartition ?bn2. \
            ?bn2 void:property ?p. \
            ?bn2 void:distinctSubjects ?x. FILTER (isBlank(?bn1) AND isBlank(?bn2))};"
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph>  void:classPartition [ void:class ?c ; void:propertyPartition [void:property ?p ; void:distinctSubjects ?x ] ] . } \
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         WITH <${DOMAIN}/graph/statistics> \
         DELETE { <$graph> void:propertyPartition ?bn. ?bn void:property ?p. ?bn void:triples ?x. } \
         WHERE { <$graph> void:propertyPartition ?bn. ?bn void:property ?p. ?bn void:triples ?x. \
          FILTER (isBlank(?bn)) };"
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> \
         INSERT INTO <${DOMAIN}/graph/statistics> { <$graph> void:propertyPartition [ void:property ?p ; void:triples ?x ] . } \
         WHERE { SELECT (COUNT(?o) AS ?x) ?p FROM <$graph> WHERE { ?s ?p ?o } GROUP BY ?p };" 
