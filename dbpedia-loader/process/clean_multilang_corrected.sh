...
for lang in ${lang_list[@]}; do
  if [[ $lang != 'fr' ]]; then
    ...
    Lang="${lang[@]^}"
    echo "============>>>>>>>>>> $lang"
    resp_count=$(run_virtuoso_cmd "SPARQL SELECT COUNT(?S) FROM <http://fr.dbpedia.org/graph/dbpedia_generic_labels> WHERE { ?S ?p ?o . FILTER NOT EXISTS { tag-fr:${Lang}FrResource oa:hasTarget ?S } };") 
