...
for graph in ${graph_list_wikidata[@]}; do
    ################### SPARQL - CLASS PARTITION - CREATE PARTITION BY CLASS
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> INSERT INTO <${DOMAIN}/graph/metadata> { <$graph> void:classPartition [void:class ?c ] . } \ WHERE { SELECT DISTINCT(?c) FROM <$graph> { ?s ?p ?o.{ select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_instance-types> WHERE { ?s rdf:type dbo:WdtHaveFrLabel. } } } };"
        ...
        ?instance ?p ?o.{ select ?instance ?c FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_instance-types> WHERE { ?s rdf:type dbo:WdtHaveFrLabel. } }
        } } GROUP BY ?class } };"
        ...
        run_virtuoso_cmd "SPARQL PREFIX void: <http://rdfs.org/ns/void#> INSERT INTO <${DOMAIN}/graph/metadata> { <$graph> void:classPartition [void:class ?c ; void:triples ?x ] . } WHERE {{ SELECT (COUNT(?p) AS ?x) ?c FROM <$graph> WHERE { ?s ?p ?o.{ select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_instance-types> WHERE { ?s rdf:type dbo:WdtHaveFrLabel. } } } GROUP BY ?c } };"
        ...
        ?s ?p ?o. { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_instance-types> WHERE { ?s rdf:type dbo:WdtHaveFrLabel. } } 
        } GROUP BY ?c } };"
        ...
        ?s ?p ?o. { select ?s ?c FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_instance-types> WHERE { ?s rdf:type dbo:WdtHaveFrLabel. } } } GROUP BY ?c ?p } };"
