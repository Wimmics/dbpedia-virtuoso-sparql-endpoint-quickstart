...
do
    ...
    INSERT { ?y rdf:type prop-fr:WdtFrResource. } \
    WHERE { SELECT ?y FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> \
    WHERE { ?s owl:sameAs ?y. FILTER NOT EXISTS { ?y rdf:type prop-fr:WdtFrResource }. \
    ...
    WHERE { ?s rdf:type prop-fr:WdtFrResource };");    
    ...
    for graph in ${graph_list[@]}; do
       ...
       WHERE { ?y owl:sameAs ?s. FILTER EXISTS { ?s rdf:type  prop-fr:WdtFrResource }} \
       } . { \
    ...
    for graph in ${graph_list[@]}; do
       ...
       WHERE { ?wkd owl:sameAs ?dbfr. FILTER EXISTS { ?dbfr rdf:type  prop-fr:WdtFrResource }} \
       } . { \
    ...
    WHERE { ?s owl:sameAs ?y. FILTER EXISTS { ?y rdf:type prop-fr:WdtFrResource } \
    } LIMIT $limit};");
   ...
   ?y rdf:type prop-fr:WdtFrResource. \
   ?s owl:sameAs ?y. \
   ?s owl:sameAs ?p. \
   FILTER (?y != ?p ) \
   } LIMIT $limit };");
        resp_todo=$(run_virtuoso_cmd "SPARQL SELECT COUNT(*) FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE {?y rdf:type prop-fr:WdtFrResource. ?s owl:sameAs ?y. ?s owl:sameAs ?p. FILTER (?y != ?p ) };");
   ...
   ?s owl:sameAs ?y \
        ...
        resp_todo2=$(run_virtuoso_cmd "SPARQL SELECT COUNT(?s) FROM <http://fr.dbpedia.org/graph/dbpedia_wikidata_sameas-all-wikis> WHERE {?y rdf:type prop-fr:WdtFrResource. ?s owl:sameAs ?y. };");
